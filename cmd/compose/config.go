...
func configCommand(p *ProjectOptions, dockerCli command.Cli) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: Adapt(func(ctx context.Context, args []string) error {
			...
			if opts.templateList {
				return runTemplateList(ctx, dockerCli, opts)
			}
			if opts.templateCreate != "" {
				return runTemplateCreate(ctx, dockerCli, opts, args)
			}
			if opts.templateRemove != "" {
				return runTemplateRemove(ctx, dockerCli, opts)
			}
			if opts.template != "" {
				return runTemplateApply(ctx, dockerCli, opts, args)
			}
	...
	flags.StringVar(&opts.template, "template", "", "Apply specified template")
	flags.StringVar(&opts.templateVars, "template-vars", "", "Variables for template (key=value format, comma-separated)")
	flags.BoolVar(&opts.templateList, "template-list", false, "List available templates")
	flags.StringVar(&opts.templateCreate, "template-create", "", "Create new template")
	flags.StringVar(&opts.templateRemove, "template-remove", "", "Remove template")
...
// TemplateInfo represents a template in the store
...
func runTemplateList(ctx context.Context, dockerCli command.Cli, opts configOptions) error {
	...
	fmt.Println("┌───────────────┬─────────────────────┬─────────────────────┬─────────────────────┐")
	fmt.Println("│ Name          │ Description         │ Created At          │ Variables           │")
	fmt.Println("├───────────────┼─────────────────────┼─────────────────────┼─────────────────────┤")

	for _, tpl := range templates {
		variables := strings.Join(tpl.Variables, ", ")
		if variables == "" {
			variables = "-"
		}
		fmt.Printf("│ %-13s │ %-19s │ %-19s │ %-19s │\n",
			tpl.Name, tpl.Description, tpl.CreatedAt, variables)
	}
...
func runTemplateCreate(ctx context.Context, dockerCli command.Cli, opts configOptions, args []string) error {
	templateName := opts.templateCreate
	...
	if err := yaml.Unmarshal(templateContent, &model); err != nil {
	...
	// Save template (simplified implementation)
	err = saveTemplate(templateName, string(templateContent), variableNames)
	if err != nil {
		return err
	}
...
func runTemplateRemove(ctx context.Context, dockerCli command.Cli, opts configOptions) error {
	templateName := opts.templateRemove

	// Remove template (simplified implementation)
	err := removeTemplate(templateName)
	if err != nil {
		return err
	}
...
func runTemplateApply(ctx context.Context, dockerCli command.Cli, opts configOptions, args []string) error {
	...
	if opts.templateVars != "" {
		for _, kv := range strings.Split(opts.templateVars, ",") {
	...
	appliedContent, err := applyTemplate(templateContent, variables)
	if err != nil {
		return err
	}
	...
	// Output result
	if opts.Output != "" {
		err = os.WriteFile(opts.Output, []byte(appliedContent), 0o666)
		if err != nil {
			return fmt.Errorf("failed to write output file: %v", err)
		}
		fmt.Printf("Template applied successfully to '%s'\n", opts.Output)
	} else {
		fmt.Println(appliedContent)
	}
...
func getAvailableTemplates() []TemplateInfo {
	// Simplified implementation - in real code, this would read from a template store
	return []TemplateInfo{
		{
			Name:        "web",
			Description: "Web application template",
			CreatedAt:   time.Now().Add(-72 * time.Hour).Format("2006-01-02 15:04:05"),
			Variables:   []string{"APP_NAME", "PORT", "IMAGE"},
		},
		{
			Name:        "api",
			Description: "API service template",
			CreatedAt:   time.Now().Add(-48 * time.Hour).Format("2006-01-02 15:04:05"),
			Variables:   []string{"API_NAME", "PORT", "DB_URL"},
		},
		{
			Name:        "database",
			Description: "Database service template",
			CreatedAt:   time.Now().Add(-24 * time.Hour).Format("2006-01-02 15:04:05"),
			Variables:   []string{"DB_NAME", "DB_USER", "DB_PASSWORD"},
		},
	}
}

func saveTemplate(name, content string, variables []string) error {
	// Simplified implementation - in real code, this would save to a template store
	// For demo purposes, just return success
	return nil
}

func removeTemplate(name string) error {
	// Simplified implementation - in real code, this would remove from a template store
	// For demo purposes, just return success
	return nil
}

func loadTemplate(name string) (string, error) {
	// Simplified implementation - in real code, this would load from a template store
...
func applyTemplate(content string, variables map[string]string) (string, error) {
	// Simplified implementation - in real code, this would use the template package
	result := content
